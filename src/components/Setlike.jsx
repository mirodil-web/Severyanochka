...
const Setlike = ({ acsiyadata, like, qoshish }) => {
    return (
        <div>
            <div className='p-2 relative'>

                {/* like */}
                <button onClick={() => qoshish(acsiyadata.id)} className={`duration-500 shadow-lg absolute right-2 p-2  ${like ? ' bg-red-400 p-2' : 'bg-slate-200/50'} rounded-full`}>
                    <img width={22} className=' rounded-lg ' height={22} src={Heart} alt="Heart icon" />
                </button>

                <Link to={`/carddata/${acsiyadata.id}`} onClick={() => qoshish(acsiyadata.id)}>
